...
for _ in range(n_datos):
    ...
    v = round(random.uniform(*velocidad_range), 2)
    ...
    Crr = round(Crr, 4)  # dejamos 4 decimales
    Rrod = round(Crr * masa_total * g, 2)

    # ----------- QUINTA FORMULA: Corrección por SLIP -----------
    S = round(random.uniform(*slip_range), 2)           # slip (0 a 0.25)
    Def = round(Dimpl * (1 + ks * S), 2)                # draft corregido
    vef = round(v * (1 - S), 2)                         # velocidad efectiva

    # ----------- SEXTA FORMULA: Fuerza Total -----------
