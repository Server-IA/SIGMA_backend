...
class MaintenanceReportViewSet(viewsets.ViewSet):
    ...
    def get_report_detail(self, request, pk=None):
        """
        Delegar la construcción del JSON al serializer MaintenanceReportDetailSerializer.
        Se mantiene la verificación de permisos y el manejo de errores.
        """
        ...
        try:
            ...
            serializer = MaintenanceReportDetailSerializer(report, context={'request': request})
            return Response({'success': True, 'message': 'Detalles del reporte obtenidos exitosamente', 'data': serializer.data}, status=status.HTTP_200_OK)
