...
from maintenance.serializers.maintenance_report_serializers import (
    ...
    # MaintenanceReportDetailSerializer removed (no usado en este módulo)
)
...
class MaintenanceReportViewSet(viewsets.ViewSet):
    ...
    def get_report_detail(self, request, pk=None):
        """
        Obtiene los detalles completos de un reporte de mantenimiento en formato JSON.
        Devuelve los mismos datos que genera el PDF pero en JSON plano, incluyendo:
          - id_maintenance_report
          - id_maintenance_scheduling
          - machinery_image (path)
          - technicians (texto con nombres y apellidos separados por comas)
          - maintenance_names (texto único con nombres de mantenimientos separados por comas)
          - spare_parts (lista con name, brand, quantity, unit_cost, total)
          - currency_symbol
          - maintenance_total, spare_parts_total, total_cost
        Se obtiene el reporte a partir del id de la programación asociado (id_maintenance_scheduling_id=pk).
        """
        ...
        if not self.check_permission(request, permission_id):
            return Response({"message": "No tiene permisos para ver detalles de reportes de mantenimiento."}, status=status.HTTP_403_FORBIDDEN)

        try:
            # Obtener el reporte con relaciones necesarias usando el id de la programación asociado
            ...
            # IDs requeridos
            report_id = getattr(report, 'id_maintenance_report', None)
            ...
            # Fecha y tipo del mantenimiento y tiempo invertido (para el JSON)
            ...
            # Recolectar y resolver todos los IDs de técnicos relevantes
            technicians_map = {}
            technicians_list = []
            try:
                tech_ids = set()

                # assigned users del reporte
                for uid in report.assigned_users.values_list('id_user', flat=True):
                    if uid:
                        tech_ids.add(uid)

                # técnicos que realizaron cada mantenimiento y orden de aparición
                ...
                # técnico asignado en la programación
                try:
                ...
                # Construir technicians_list manteniendo orden: técnicos por mantenimiento (orden de aparición), luego assigned scheduling
                seen = set()
            ...
            for rel in getattr(report, 'maintenance_relations').all():
                ...
                # resolver nombre del técnico que realizó este mantenimiento
            ...
            # Totales
            try:
                total_general = float(getattr(report, 'total_cost', maintenance_total + spare_total) or (maintenance_total + spare_total))
            except Exception:
                total_general = maintenance_total + spare_total

            # Si aún no hay técnicos resueltos, intentar inferir desde scheduling.assigned_technician
            if not technicians_list:
                try:
                    sched_tech = getattr(scheduling, 'assigned_technician', None)
                    if sched_tech is not None:
                        parts = [
                            getattr(sched_tech, 'name', None) or getattr(sched_tech, 'first_name', None) or '',
                            getattr(sched_tech, 'first_last_name', None) or getattr(sched_tech, 'last_name', None) or '',
                            getattr(sched_tech, 'second_last_name', None) or getattr(sched_tech, 'secondLastName', None) or ''
                        ]
                        full = ' '.join([p.strip() for p in parts if p])
                        if full:
                            technicians_list = [full]
                except Exception:
                    technicians_list = []

            # Convertir maintenance_names a texto único separado por comas
            maintenance_names_text = ', '.join(maintenance_names)

            # technicians como texto único
            technicians_text = ', '.join(technicians_list) if technicians_list else ''

            # Información de la maquinaria (añadida al JSON de salida)
            ...
            data = {
                'id_maintenance_report': report_id,
            ...
            return Response({'success': True, 'message': 'Detalles del reporte obtenidos exitosamente', 'data': data}, status=status.HTTP_200_OK)

        except MaintenanceReport.DoesNotExist:
            return Response({"success": False, "message": "Reporte de mantenimiento no encontrado"}, status=status.HTTP_404_NOT_FOUND)

        except Exception as e:
            logger.error(f"Error obteniendo detalles del reporte {pk}: {str(e)}")
            return Response({"success": False, "message": "Error al obtener los detalles del reporte", "details": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
