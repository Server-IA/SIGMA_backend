...
class MachineryToleranceThresholdsCreateSerializer(serializers.Serializer):
    """
    Serializer para crear configuraciones de tolerancia, fallos OBD y tipos de eventos para una maquinaria.
    Maneja la creación masiva de registros relacionados con validaciones específicas.
    ...
    def validate_tolerance_thresholds(self, value):
        ...
        # 4) Validar rangos de threshold con respecto a los parámetros
        for item in value:
            id_parameter = item.get('id_parameter')
    ...
    def validate(self, data):
        ...
        if not Machinery.objects.filter(id_machinery=id_machinery.id_machinery).exists():
            raise ValidationError({"id_machinery": "La maquinaria especificada no existe."})
    ...
    def create(self, validated_data):
        """
        Crear todos los registros relacionados en una transacción
        """
        ...
        try:
            ...
            for threshold_data in validated_data.get('tolerance_thresholds', []):
                ...
                created_records['tolerance_thresholds'].append(threshold)

            # Crear obd_fault_machinery
            for obd_data in validated_data.get('obd_fault_machinery', []):
                ...
                created_records['obd_fault_machinery'].append(obd_fault)

            # Crear event_type_machinery
            for event_data in validated_data.get('event_type_machinery', []):
                ...
                created_records['event_type_machinery'].append(event_type)
            ...
            raise ValidationError(f"Error al crear los registros: {str(e)}")
