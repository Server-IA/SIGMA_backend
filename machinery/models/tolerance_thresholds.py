from django.db import models

class ToleranceThresholds(models.Model):
    id = models.AutoField(primary_key=True)
    id_machinery = models.ForeignKey('machinery.Machinery', on_delete=models.PROTECT, null=False, blank=False, db_column='id_machinery')
    id_parameter = models.ForeignKey('machinery.Parameters', on_delete=models.PROTECT, null=False, blank=False, db_column='id_parameter')
    minimum_threshold = models.FloatField(null=True, blank=True)
    maximum_threshold = models.FloatField(null=True, blank=True)
    id_maintenance = models.ForeignKey('maintenance.Maintenance', on_delete=models.PROTECT, null=True, blank=True, db_column='id_maintenance')
    alert_enabled = models.BooleanField(null=False, blank=False)

    class Meta:
        db_table = 'tolerance_thresholds'